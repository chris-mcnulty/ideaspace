...
- **Workspace Access Controls**: Enforces workspace status-based and guest permission-based access restrictions for participant routes. Middleware validates workspace status (open/closed), guest access permissions, and participant session integrity before allowing access to ideation, voting, ranking, and marketplace endpoints. Includes optional `resultsPublicAfterClose` flag allowing read-only results access even after workspace closure (configurable by facilitators). Admins and facilitators bypass all restrictions. Returns structured error codes (WORKSPACE_CLOSED, WORKSPACE_NOT_OPEN, GUEST_ACCESS_DISABLED, GUEST_ACCESS_REVOKED, NO_ACCESS) for client-side error handling.
