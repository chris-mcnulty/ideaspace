...
- **Manual Category Management**: A facilitator-controlled system for creating custom categories with names and colors, separate from AI categorization. Facilitators can create categories via dialog, and manually assign notes to categories via dropdown on each note card. Uses FK relationship (manualCategoryId) to ensure referential integrity, preventing collision with AI-generated category labels. WebSocket broadcasts ensure real-time updates across clients.
