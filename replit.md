...
## Backlog
- [x] Database performance pass - Eliminated N+1 patterns in hot endpoints (/api/projects, /api/my-organizations, /api/my-projects, /api/my-projects/detailed, /api/my-workspaces) by adding batched IStorage methods (getOrganizationsByIds, getProjectsByIds, getProjectsByOrganizations, getSpacesByOrganizations, getSpacesByProjects, getSpacesByIds, getNoteCountsBySpaces, getParticipantCountsBySpaces) using inArray + GROUP BY. Added idempotent `ensurePerformanceIndexes()` startup migration creating CREATE INDEX IF NOT EXISTS on hot foreign keys (notes/votes/rankings/marketplace_allocations/survey_responses/survey_questions/categories/participants by space, spaces by org/project, projects by org, project_members by user/project, space_facilitators by user/space). Stripped debug console.log noise from server/routes.ts.
