...
(async () => {
  ...
  // In development, use port 5001 to avoid conflict with Orion on port 5000
  // In production, use the PORT env variable (defaults to 5000)
  const port = process.env.NODE_ENV === 'development' ? 5001 : parseInt(process.env.PORT || '5000', 10);
  server.listen({
    port,
    host: "0.0.0.0",
    reusePort: true,
